...
def test_setters(
    ...
    resp = strategy.harvestTrigger(0, {"from": gov})
    print("\nShould we harvest? Should be true.", resp)
    assert resp == True

    strategy.setForceHarvestTriggerOnce(False, {"from": gov})
    resp = strategy.harvestTrigger(2000e18, {"from": gov})
    print("\nShould we harvest? Should be false.", resp)
    assert resp == False
